# Sgetrf/Sgetrs/SgetriBatched/SmatinvBatched 接口开发任务书

## 1. 任务概述

在昇腾 NPU 上基于 AscendC/CATLASS，开发稠密线性求解功能以及配套的稠密批量求逆功能，具体 API 包括 `aclsolverSgetrf`、`aclsolverSgetrs`、`aclsolverSgetriBatched`、`aclsolverSmatinvBatched`。接口参考 NVIDIA cuSolver DN 稠密线性求解接口（LU 分解 `getrf`、三角求解 `getrs`）以及配套的稠密批量求逆接口（`getriBatched`、`matinvBatched`）。要求接口、功能、参数、约束与对标 CUDA 接口对齐，精度达到 CANN 生态算子开源标准。完成设计、开发、测试全流程，验收通过后合入 https://gitcode.com/cann/ops-solver 。

本任务适配硬件为 **Atlas 800I A2 / Atlas 800I A3**（下文简称 A2 / A3）。四个接口作为**同一社区任务**一并交付，不允许拆分验收。本任务**不交付**单矩阵 `aclsolverSgetri`。
...
仓库 https://gitcode.com/cann/ops-solver 中已有 `aclsolverSgetrf` / `aclsolverSgetri` 原型，当前与对标接口**不对齐**（Host 指针、`lda` 强制等于 `n`、`info` 未写入、无 workspace 查询、无 `Sgetrs` / 批量接口）。本任务须将 `aclsolverSgetrf` 升级为下文定义的公开接口，并新增 `aclsolverSgetrs`、`aclsolverSgetriBatched`、`aclsolverSmatinvBatched`；不得将上述简化语义作为最终交付。已有 `aclsolverSgetri` 不在本任务范围内。

#### 数学公式

1. **Sgetrf（LU 分解，带部分主元）**

$$
P \cdot A = L \cdot U
$$

其中 \(A\) 为 \(m \times n\) 单精度实矩阵，\(P\) 为置换矩阵，\(L\) 为单位下三角（或梯形）矩阵，\(U\) 为上三角（或梯形）矩阵。分解结果原地写入 \(A\)：对角线及上方为 \(U\)，严格下方为 \(L\) 的非对角元。`devIpiv` 为 1-based 主元序列：第 \(i\) 行与第 `devIpiv(i)` 行交换。若 `devIpiv == nullptr`，则不做选主元，分解为 \(A = L \cdot U\)（数值不稳定，语义对齐 `cusolverDnSgetrf`）。

2. **Sgetrs（基于 LU 因子求解）**

$$
\mathrm{op}(A) \cdot X = B
$$

其中 \(A\) 为已由 `Sgetrf` 分解的 \(n \times n\) 方阵，\(B\) 为 \(n \times \mathrm{nrhs}\) 右端项，求解后 \(X\) 原地覆盖 \(B\)。\(\mathrm{op}(A)\) 由 `trans` 指定：`N` 为不转置，`T` / `C` 对实数均为转置。

3. **SgetriBatched（批量求逆，基于已分解 LU）**

对 \(i = 0, \ldots, \mathrm{batchSize}-1\)：

$$
C_i = A_i^{-1}
$$

其中 \(A_i\) 必须先完成 LU 分解（可用循环调用 `aclsolverSgetrf` 构造因子；本任务不强制交付 `SgetrfBatched`）。求逆为 **out-of-place**：\(C_i\) 与 \(A_i\) 内存不得重叠。

4. **SmatinvBatched（小矩阵直接批量求逆）**

对原始矩阵直接求逆，功能等价 `getrf + getri` 的短路实现。**仅支持 \(n \le 32\)**；\(n > 32\) 必须返回非法参数，不得静默转调 `SgetriBatched`（对齐 `cublasSmatinvBatched`）。若 \(A_i\) 奇异，`info[i]` 报告奇异位置，语义同 `getrf`。
...
3. `Sgetrf` 须提供 `_bufferSize` 查询接口，调用方按返回的 `Lwork` 分配 Device workspace 后再执行计算。
...
- Kernel：基于 AscendC/CATLASS 实现 LU / TRSM / 批量求逆
...
参数名、顺序、含义、Device/Host 归属不得擅自增删或调换。维数类型与对标 CUDA legacy API 一致，使用 `int`（32-bit）。批量接口的 `Aarray` / `Carray` / `A` / `Ainv` 为 **Device 上的指针数组**（array of device pointers），不是 `[batch, n, n]` 连续张量。
...
| 接口 | 维数 | 其他 |
|---|---|---|
| Sgetrf | m, n ∈ [0, 4096]，须覆盖方阵与矩形（m≠n） | lda 允许大于 m 的 padding |
| Sgetrs | n ∈ [0, 4096]，nrhs ∈ [0, 128] | trans 覆盖 N/T/C |
| SgetriBatched | n ∈ [1, 256]，batchSize ∈ [1, 3000] | 须覆盖 lda/ldc padding |
| SmatinvBatched | n ∈ [1, 32]，batchSize ∈ [1, 3000] | n=33 必须报错 |
...
- **适配硬件**：Atlas 800I A2（Ascend 910B3 / 910B4）、Atlas 800I A3（Ascend 910_93）
...
- **版本记录**：自测报告须记录 CANN 版本、NPU 具体型号（910B3 或 910B4 或 910_93）、驱动版本、CUDA / cuSolver / cuBLAS 版本

功能和精度须提交 A2 与 A3 测试结果；性能须分别给出 910B4（若仅有 910B3 须注明）与 910_93 的数据。
...
2. NPU 输出（`Sgetrf` 还原 \(P^{-1}LU\) 后与原 A 比较，或 `Sgetrs` 的 X、`SgetriBatched` / `SmatinvBatched` 的 \(A^{-1}\) 及 \(A A^{-1}\) 相对 I）须满足生态算子开源精度标准：https://gitcode.com/cann/opbase/blob/master/docs/zh/ops_precision_standard/experimental_standard.md
3. FLOAT32 混合容差如下：
...
1. 性能对标为 NVIDIA A100 上同 shape、同 dtype（FLOAT32）、同列主序、同等预热与计时口径下的 CUDA 接口：
   - Sgetrf：`cusolverDnSgetrf`（含一次 bufferSize，不计在 kernel 耗时内）
   - Sgetrs：`cusolverDnSgetrs`（getrf 时间不计入本接口）
   - SgetriBatched：`cublasSgetriBatched`
   - SmatinvBatched：`cublasSmatinvBatched`
2. 达标判据：对下表每一个 case，NPU kernel 耗时满足 \(T_{\mathrm{NPU}} \le T_{\mathrm{A100}} / 0.8\)，即性能倍率 ≥ 0.8 倍 A100。Atlas 800I A2（须明确 910B3 或 910B4）与 Atlas 800I A3（910_93）均须达标。
3. 每个 case 至少预热 10 次、正式采样 30 次，报告中位数；每轮须 Device 同步后计时。不含首次编译、数据生成、H2D/D2H。workspace / 指针数组在正式采样期间复用。
4. 开发者须在 A100 上实测填入 \(T_{\mathrm{A100}}\)，并在报告中给出 CUDA 版本与采集脚本。下表 \(T_{\mathrm{A100}}\) 列为待测占位。

| 编号 | 接口 | 规格 | GPU A100 性能 | NPU 耗时 | 目标 |
|---|---|---|---|---|---|
| P-01 | Sgetrf | m=n=1024，lda=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-02 | Sgetrf | m=n=4096，lda=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-03 | Sgetrf | m=2048，n=1024，lda=m | 待测 | 待测 | ≥ 0.8×A100 |
| P-04 | Sgetrs | n=1024，nrhs=1，trans=N | 待测 | 待测 | ≥ 0.8×A100 |
| P-05 | Sgetrs | n=4096，nrhs=32，trans=N | 待测 | 待测 | ≥ 0.8×A100 |
| P-06 | Sgetrs | n=2048，nrhs=8，trans=T | 待测 | 待测 | ≥ 0.8×A100 |
| P-07 | SgetriBatched | n=32，batchSize=1024，lda=ldc=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-08 | SgetriBatched | n=128，batchSize=128，lda=ldc=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-09 | SmatinvBatched | n=16，batchSize=1024，lda=lda_inv=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-10 | SmatinvBatched | n=32，batchSize=256，lda=lda_inv=n | 待测 | 待测 | ≥ 0.8×A100 |

全部 10 条在所申报的 A2 芯片与 A3 上均须达标，未达标不予验收。

### 3.4 内存要求

对标 cuSolver / cuBLAS 同等规格：

1. `Sgetrf` 的 Device workspace 不超过 `bufferSize` 返回值；禁止额外分配未计入 `Lwork` 的隐蔽大块 Device 内存。`bufferSize` 建议不超过 \(\max(1, m \times n)\) 量级（对齐 cuSolver getrf 大 workspace 实现口径），并在设计文档中给出公式。
...
本任务使用 ops-solver 仓测试工程（https://gitcode.com/cann/ops-solver/tree/master/test ）及 AscendOpTest 工具（https://gitcode.com/HIT1920/AscendOpTest ）进行自验。请根据本任务给出的自测用例和测试指导完成自测，并输出自测报告。自测用例目录：./sgetrf_family_testCase/

NPU 接口与 CUDA 接口参数序列一致（见 2.3），无需额外映射。差异仅在：命名前缀 `aclsolver` vs `cusolverDn`/`cublas`，以及 `aclsolverOperation_t` vs `cublasOperation_t`（枚举值 0/1/2 对齐）。

功能自验至少覆盖：

| 类别 | 必测场景 |
|---|---|
| 基础功能 | 方阵 getrf；getrf+getrs 求解；SgetriBatched 求逆；matinv n≤32 |
...
3. ops-solver 仓库：https://gitcode.com/cann/ops-solver ；
4. 生态算子开源精度标准：https://gitcode.com/cann/opbase/blob/master/docs/zh/ops_precision_standard/experimental_standard.md ；
5. AscendOpTest：https://gitcode.com/HIT1920/AscendOpTest ；
6. Ascend C算子开发文档：https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/850/opdevg/Ascendcopdevg/atlas_ascendc_map_10_0002.html ；
7. 算子开发接口文档：https://www.hiascend.com/document/detail/zh/canncommercial/850/API/ascendcopapi/atlasascendc_api_07_0003.html ；
8. Ascend C在线课程：https://www.hiascend.com/developer/courses/detail/1691696509765107713 ；
9. CATLASS：https://gitcode.com/cann/catlass ；
10. ops-solver 现有单精度原型文档：https://gitcode.com/cann/ops-solver/blob/master/docs/zh/sgetrf.md 。

## 7. 特别注意事项

1. 本任务四个接口必须全部交付：Sgetrf、Sgetrs、SgetriBatched、SmatinvBatched。缺一不可。本任务不交付单矩阵 `aclsolverSgetri`。
...
4. `SmatinvBatched` 在 n>32 时必须报错，不得转调 `SgetriBatched`。
5. `SgetriBatched` 的输入是已分解的 LU 因子 + 主元，不是未分解的原始 A；端到端批量求逆须先 getrf 再 getriBatched。`SmatinvBatched` 直接对原始矩阵求逆。
6. 性能门禁为 0.8 倍 A100，A2（须写明 910B3 或 910B4）与 A3（910_93）均须达标。
