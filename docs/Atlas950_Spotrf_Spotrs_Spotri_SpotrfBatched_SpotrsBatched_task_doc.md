# Atlas 950 单精度实数Cholesky分解、求解和批量接口 任务书

## 1. 任务概述

在昇腾 NPU 上基于 AscendC/CATLASS，开发稠密正定线性求解功能以及配套批量接口，具体 API 包括 `aclsolverSpotrf`、`aclsolverSpotrs`、`aclsolverSpotri`、`aclsolverSpotrfBatched`、`aclsolverSpotrsBatched`。接口参考 NVIDIA cuSolver DN 稠密 Cholesky 接口（分解 `potrf`、求解 `potrs`、求逆 `potri`）以及批量接口（`potrfBatched`、`potrsBatched`）。要求接口、功能、参数、约束与对标 CUDA 接口对齐，精度达到 CANN 生态算子开源标准。完成设计、开发、测试全流程，验收通过后合入 https://gitcode.com/cann/ops-solver 。

本任务适配硬件为 **Ascend 950PR（Atlas 950）**。五个接口作为**同一社区任务**一并交付，不允许拆分验收。ops-solver 仓库当前无对应 Cholesky 原型，须全新实现。
...
|---|---|---|
| `aclsolverSpotrf` / `aclsolverSpotrf_bufferSize` | `cusolverDnSpotrf` / `cusolverDnSpotrf_bufferSize` | https://docs.nvidia.com/cuda/cusolver/index.html#cuSolverDN-legacy-api |
| `aclsolverSpotrs` | `cusolverDnSpotrs` | https://docs.nvidia.com/cuda/cusolver/index.html#cuSolverDN-legacy-api |
| `aclsolverSpotri` / `aclsolverSpotri_bufferSize` | `cusolverDnSpotri` / `cusolverDnSpotri_bufferSize` | https://docs.nvidia.com/cuda/cusolver/index.html#cuSolverDN-legacy-api |
| `aclsolverSpotrfBatched` | `cusolverDnSpotrfBatched` | https://docs.nvidia.com/cuda/cusolver/index.html#cusolverdn-potrfbatched |
| `aclsolverSpotrsBatched` | `cusolverDnSpotrsBatched` | https://docs.nvidia.com/cuda/cusolver/index.html#cusolverdn-potrsbatched |

#### 数学公式

1. **aclsolverSpotrf（Cholesky 分解）**
...
其中 A 为 n × n 单精度实对称正定矩阵。仅处理 `uplo` 指定的三角部分，分解结果原地覆盖该三角部分。对齐 `cusolverDnSpotrf`：未使用的另一半三角可作为 workspace 被破坏。

2. **aclsolverSpotrs（基于 Cholesky 因子求解）**

```
A · X = B
```

其中 A 为已由 `aclsolverSpotrf` 分解的因子，B 为 n × nrhs 右端项，求解后 X 原地覆盖 B。`uplo` 必须与分解时一致。

3. **aclsolverSpotri（基于 Cholesky 因子求逆）**

```
A^{-1} · A = I
```

输入 A 为 `aclsolverSpotrf` 得到的三角因子，输出原地覆盖为对称逆矩阵的对应三角部分。语义对齐 `cusolverDnSpotri`。

4. **aclsolverSpotrfBatched（批量 Cholesky 分解）**

对 i = 0, ..., batchSize-1，对每个 A[i] 执行与 `aclsolverSpotrf` 相同的分解。`infoArray[i] = k > 0` 表示第 i 个矩阵的 k 阶顺序主子式不正定。

5. **aclsolverSpotrsBatched（批量 Cholesky 求解）**
...
A[i] 须先由 `aclsolverSpotrfBatched` 分解。求解原地覆盖 B[i]。**仅支持 nrhs = 1**（对齐 `cusolverDnSpotrsBatched`）。`info` 为标量：仅用于报告非法参数（`-i`），正定性由 `potrfBatched` 的 `infoArray` 反映。
...
3. `aclsolverSpotrf` / `aclsolverSpotri` 须提供 `_bufferSize`；计算接口带 `Lwork`，对齐 `cusolverDnSpotrf` / `cusolverDnSpotri`。
4. `devInfo` / `info` 必须真实写入：`0` 成功；`-i` 表示第 i 个参数非法（不计 handle）；`i > 0` 表示第 i 阶顺序主子式不正定（potrf / potri）或分解失败。批量 `potrf` 对每个 batch 独立写 `infoArray[i]`。
...
非法类型 / shape / dtype / 排布 / 值域须返回 `ACLSOLVER_STATUS_INVALID_VALUE`，并在 `devInfo`/`info` 可写时写入 `-i`。空问题（n=0 / nrhs=0 / batchSize=0）成功返回。`aclsolverSpotrsBatched` 仅支持 `nrhs = 1`，`nrhs ≠ 1` 且 n>0 时必须报错。

#### 2.4.1 aclsolverSpotrf / aclsolverSpotrf_bufferSize
...
| A | 输入/输出（原地） | 对称正定矩阵；输出为 Cholesky 因子 | tensor | FLOAT32 | 列主序 ND | lda × n | 有限浮点；允许构造非正定矩阵以验证 info | 空指针且 n>0 时报错；lda < max(1,n) 报错 |
...
| Workspace | 输入（仅 potrf） | Device 工作空间 | tensor | FLOAT32 | ND | [Lwork] | - | 指针为空且 Lwork>0 时报错 |
| devInfo | 输出 | 分解信息 | tensor | INT32 | ND | [1] | 0 / -i / i | 空指针报错 |

#### 2.4.2 aclsolverSpotrs
...
| A | 输入 | Spotrf 得到的三角因子，只读 | tensor | FLOAT32 | 列主序 ND | lda × n | 有限浮点 | 空指针且 n>0 时报错 |
| lda | 输入 | A 的 leading dimension | scalar | int | - | - | lda >= max(1,n) | 不满足报错 |
| B | 输入/输出（原地） | 输入为 B，输出为 X | tensor | FLOAT32 | 列主序 ND | ldb × nrhs | 有限浮点 | 空指针且 n,nrhs>0 时报错；ldb < max(1,n) 报错 |
| ldb | 输入 | B 的 leading dimension | scalar | int | - | - | ldb >= max(1,n) | 不满足报错 |
| devInfo | 输出 | 求解信息 | tensor | INT32 | ND | [1] | 0 / -i | 空指针报错 |

#### 2.4.3 aclsolverSpotri / aclsolverSpotri_bufferSize
...
| uplo | 输入 | 须与 potrf 一致 | attr | aclsolverFillMode_t | - | - | LOWER、UPPER | 非法枚举报错 |
| n | 输入 | 方阵阶数 | scalar | int | - | - | n >= 0 | n < 0 报错 |
| A | 输入/输出（原地） | 输入为三角因子，输出为逆矩阵对应三角 | tensor | FLOAT32 | 列主序 ND | lda × n | 有限浮点 | 空指针且 n>0 时报错 |
| lda | 输入 | A 的 leading dimension | scalar | int | - | - | lda >= max(1,n) | 不满足报错 |
| Lwork | 输出（仅 bufferSize） / 输入（仅 potri） | workspace 元素个数 | scalar | int* / int | - | [1] | Lwork >= 0 | bufferSize 时指针为空报错 |
| Workspace | 输入（仅 potri） | Device 工作空间 | tensor | FLOAT32 | ND | [Lwork] | - | 指针为空且 Lwork>0 时报错 |
| devInfo | 输出 | 求逆信息 | tensor | INT32 | ND | [1] | 0 / -i / i | 空指针报错 |

#### 2.4.4 aclsolverSpotrfBatched
...
| Aarray | 输入/输出（原地） | Device 指针数组 | list(tensor) | FLOAT32 | 列主序，每个矩阵 lda × n | 指针数组长度 batchSize | 有限浮点 | 空指针、lda < max(1,n) 报错 |
...
| Aarray | 输入 | Device 指针数组，Cholesky 因子 | list(tensor) | FLOAT32 | 列主序，每个矩阵 lda × n | 指针数组长度 batchSize | 有限浮点 | 空指针报错 |
| lda | 输入 | 每个 Aarray[i] 的 leading dimension | scalar | int | - | - | lda >= max(1,n) | 不满足报错 |
| Barray | 输入/输出（原地） | Device 指针数组，输入 B、输出 X | list(tensor) | FLOAT32 | 列主序，每个矩阵 ldb × nrhs | 指针数组长度 batchSize | 有限浮点 | 空指针、ldb < max(1,n) 报错 |
...
| 接口 | 维数 | 其他 |
|---|---|---|
| aclsolverSpotrf / aclsolverSpotri | n ∈ [0, 4096] | uplo 覆盖 LOWER/UPPER；lda padding |
| aclsolverSpotrs | n ∈ [0, 4096]，nrhs ∈ [0, 128] | uplo 覆盖 LOWER/UPPER |
| aclsolverSpotrfBatched | n ∈ [1, 256]，batchSize ∈ [1, 3000] | uplo 覆盖 LOWER/UPPER |
| aclsolverSpotrsBatched | n ∈ [1, 256]，batchSize ∈ [1, 3000]，nrhs=1 | nrhs=2 必须报错 |
...
1. 对标接口见 2.1 节。数值正确性以更高精度 CPU 参考为单标杆：FLOAT32 计算的 golden 使用 FLOAT64（NumPy/SciPy `float64` 的 `cholesky` / `cho_solve` / `inv`，或等价 LAPACK `dpotrf`/`dpotrs`/`dpotri`）。
2. NPU 输出（`aclsolverSpotrf` 还原 L Lᵀ 或 Uᵀ U 后与原 A 的指定三角比较，或 `aclsolverSpotrs` 的 X、`aclsolverSpotri` 的 A⁻¹ 及 A A⁻¹ 相对 I、批量接口对应输出）须满足生态算子开源精度标准：https://gitcode.com/cann/opbase/blob/master/docs/zh/ops_precision_standard/experimental_standard.md
3. FLOAT32 混合容差如下：
...
5. 覆盖对角占优 SPD、随机 SPD（A = BᵀB + n I）、故意构造的非正定矩阵；INF/NAN 按精度标准文档对应规则验收。
6. 确定性：合法 SPD 用例重复执行须 bit-wise 一致。

### 3.3 性能要求

1. 性能对标为 NVIDIA A100 上同 shape、同 dtype（FLOAT32）、同列主序、同等预热与计时口径下的 CUDA 接口：
   - aclsolverSpotrf：`cusolverDnSpotrf`（含一次 bufferSize，不计在 kernel 耗时内）
   - aclsolverSpotrs：`cusolverDnSpotrs`（potrf 时间不计入本接口）
   - aclsolverSpotri：`cusolverDnSpotri`（potrf 时间不计入本接口）
...
|---|---|---|---|---|---|
| P-01 | aclsolverSpotrf | n=1024，uplo=LOWER，lda=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-02 | aclsolverSpotrf | n=4096，uplo=LOWER，lda=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-03 | aclsolverSpotrf | n=2048，uplo=UPPER，lda=n | 待测 | 待测 | ≥ 0.8×A100 |
| P-04 | aclsolverSpotrs | n=1024，nrhs=1，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-05 | aclsolverSpotrs | n=4096，nrhs=32，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-06 | aclsolverSpotrs | n=2048，nrhs=8，uplo=UPPER | 待测 | 待测 | ≥ 0.8×A100 |
| P-07 | aclsolverSpotri | n=1024，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-08 | aclsolverSpotri | n=4096，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-09 | aclsolverSpotrfBatched | n=32，batchSize=1024，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-10 | aclsolverSpotrfBatched | n=128，batchSize=128，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-11 | aclsolverSpotrsBatched | n=32，batchSize=1024，nrhs=1，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
| P-12 | aclsolverSpotrsBatched | n=128，batchSize=128，nrhs=1，uplo=LOWER | 待测 | 待测 | ≥ 0.8×A100 |
...
1. `aclsolverSpotrf` / `aclsolverSpotri` 的 Device workspace 不超过 `bufferSize` 返回值；禁止额外分配未计入 `Lwork` 的隐蔽大块 Device 内存，并在设计文档中给出公式。
...
本任务使用 ops-solver 仓测试工程（https://gitcode.com/cann/ops-solver/tree/master/test ）及 AscendOpTest 工具（https://gitcode.com/HIT1920/AscendOpTest ）进行自验。请根据本任务给出的自测用例和测试指导完成自测，并输出自测报告。自测用例目录：./spotrf_family_950_testCase/

NPU 接口与 CUDA 接口参数序列一致（见 2.3），无需额外映射。差异仅在：命名前缀 `aclsolver` vs `cusolverDn`，以及 `aclsolverFillMode_t` vs `cublasFillMode_t`（枚举值 0/1 对齐）。
...
|---|---|---|
| A（精度主路径，须 SPD） | 50%：随机 B 均匀 [-5,5] 后 A = BᵀB + n I；50%：正态同样构造。另用 10% 用例构造非正定（对角减大数）用于 info | n 覆盖 2 的幂与 2 的幂-1 |
| B | 均匀/正态各 50% | nrhs 覆盖 1、8、32、128 及边界 0；batched 仅 nrhs=1 |
...
docs/zh/spotrs.md
docs/zh/spotri.md
docs/zh/spotrf_batched.md
docs/zh/spotrs_batched.md
docs/api_list.md
```
...
1. 本任务五个接口必须全部交付：aclsolverSpotrf、aclsolverSpotrs、aclsolverSpotri、aclsolverSpotrfBatched、aclsolverSpotrsBatched。缺一不可。
...
4. `aclsolverSpotrsBatched` 仅支持 nrhs=1，nrhs≠1 必须报错。
5. `aclsolverSpotrs` / `aclsolverSpotri` / `aclsolverSpotrsBatched` 的输入是已分解的 Cholesky 因子，不是未分解的原始 A。
6. 对齐 cuSolver：未使用的另一半三角可作为 workspace 被破坏，验收时只比较 `uplo` 指定的三角部分。
