...
│   ├── README.md           // 说明文档
│   └── cgetri_batched_test.cpp       // 算子调用样例
```

## 算子描述

- 算子功能：  
    CgetriBatched算子计算批量复数矩阵的逆矩阵，对应的数学表达式为：
    $$A^{-1}A = I$$
...
- 算子规格：
    ...
    <tr>
        ...
        <td colspan="5" align="center">CgetriBatched</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td colspan="5" align="center">cgetri_batched_kernel</td>
    </tr>
    </table>
...
- 样例执行
  ```bash
  bash build.sh --ops=cgetri_batched --run # --ops=<算子名> --run可选参数，执行测试样例
  ```
  执行结果如下，说明精度对比成功。
  ```bash
