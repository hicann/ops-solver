...
set(OPS_SOLVER_VERSION_INFO "${CMAKE_BINARY_DIR}/version.info")
add_custom_target(gen_ops_solver_version_info ALL
  COMMAND ${CMAKE_COMMAND} -E make_directory "${CMAKE_BINARY_DIR}"
  COMMAND bash "${CMAKE_SOURCE_DIR}/scripts/util/gen_version_info.sh"
    "${ASCEND_INSTALL_DIR}"
    "${CMAKE_BINARY_DIR}"
    "${OPS_SOLVER_RUN_VERSION}"
  BYPRODUCTS "${OPS_SOLVER_VERSION_INFO}"
  COMMENT "Generating version.info for ops_solver run package"
)

# 安装 version.info 到 run 包的 share/info/ops_solver 目录
install(FILES "${OPS_SOLVER_VERSION_INFO}"
  DESTINATION share/info/ops_solver
...
function(pack _soc_version)
  ...
  set(CPACK_PACKAGE_VERSION "${OPS_SOLVER_RUN_VERSION}")
  set(CPACK_PACKAGE_FILE_NAME "cann-${CPACK_CHIP_SHORT}-ops-solver_${OPS_SOLVER_RUN_VERSION}_linux-${PACK_ARCH}")
