...
/*!
 * \file cmatinv_batched_kernel.cpp
 * \brief
 */
...
class CmatinvBatchedAIV
{
   public:
    ...
    __aicore__ inline void Init(GM_ADDR dA, GM_ADDR dUniReal, GM_ADDR dUniImag, GM_ADDR dOffset, GM_ADDR dAinv,
    ...
    __aicore__ inline void CopyCmatBatchGmToUb(LocalTensor<T> dst, GlobalTensor<T> src, uint32_t matColSize,
                                               uint32_t batchSize);
    __aicore__ inline void CopyUniMatBatchGmToUb(LocalTensor<float> dst, GlobalTensor<float> src, uint32_t matColSize,
                                                 uint32_t batchSize, uint32_t isComplex);
    __aicore__ inline void CopyVecGmToUbUint32(LocalTensor<uint32_t> dst, GlobalTensor<uint32_t> src, uint32_t len);
    __aicore__ inline void CopyVecGmToUb(LocalTensor<float> dst, GlobalTensor<float> src, uint32_t len);
    __aicore__ inline void CopyVecUbToGm(GlobalTensor<T> dst, LocalTensor<T> src, uint32_t len);
    ...
    __aicore__ inline void RealScalarVecBatchMul(LocalTensor<float> dst, LocalTensor<float> src0,
                                                 LocalTensor<float> src1, uint32_t oneMatOffset);
    __aicore__ inline void RealScalarVecBatchSub(LocalTensor<float> dst, LocalTensor<float> src, uint32_t oneMatOffset);
    __aicore__ inline void RealVecScalarBatchDiv(LocalTensor<float> dst, LocalTensor<float> src0,
                                                 LocalTensor<float> src1, uint32_t oneMatOffset);

    __aicore__ inline void ComplexScalarVecBatchMul(LocalTensor<float> matReal, LocalTensor<float> matImag,
                                                    uint32_t oneMatOffset);
    __aicore__ inline void ComputeAndUpdateAj(LocalTensor<float> AjReal, LocalTensor<float> AjImag,
                                              uint32_t oneMatOffset);
    __aicore__ inline void ComplexVecScalarBatchDiv(LocalTensor<float> vecReal, LocalTensor<float> vecImag,
                                                    uint32_t oneMatOffset);
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::Init(GM_ADDR dA, GM_ADDR dUniReal, GM_ADDR dUniImag, GM_ADDR dOffset,
                                                  GM_ADDR dAinv, GM_ADDR workSpace, GM_ADDR tilingGm)
{
    SetMaskNorm();
    SetAtomicNone();
...
{
    uint32_t alignedBatchSize =
        (this->batchNumPerRepeat + ELENUM_REPEAT_FP32 - 1) / ELENUM_REPEAT_FP32 * ELENUM_REPEAT_FP32;  // 256 bytes
...
__aicore__ inline void CmatinvBatchedAIV<T>::ParseTilingData(GM_ADDR tilingGm)
{
    ...
    this->n =
        (*(__gm__ uint32_t *)((__gm__ uint8_t *)tilingBuf + locId * sizeof(uint32_t)));  // num of complex elements
    locId++;
    this->offset = (*(__gm__ uint32_t *)((__gm__ uint8_t *)tilingBuf + locId * sizeof(uint32_t) +
                                         sizeof(uint32_t) * this->coreIdx));
    locId += MAX_CORE_CNT;
    this->calNum = (*(__gm__ uint32_t *)((__gm__ uint8_t *)tilingBuf + locId * sizeof(uint32_t) +
                                         sizeof(uint32_t) * this->coreIdx));
}
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::CopyCmatBatchGmToUb(LocalTensor<T> dst, GlobalTensor<T> src,
                                                                 uint32_t matColSize, uint32_t batchSize)
{
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::CopyVecGmToUbUint32(LocalTensor<uint32_t> dst, GlobalTensor<uint32_t> src,
                                                                 uint32_t len)
{
    uint16_t blockCnt = 1;
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::CopyVecGmToUb(LocalTensor<float> dst, GlobalTensor<float> src,
                                                           uint32_t len)
{
    uint16_t blockCnt = 1;
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::CopyVecUbToGm(GlobalTensor<T> dst, LocalTensor<T> src, uint32_t len)
{
    uint16_t blockCnt = 1;
...
__aicore__ inline void CmatinvBatchedAIV<T>::ComputeComplexScalarOffset()
{
    ...
    uint32_t alignedBatchSize = (batchSize + ELENUM_REPEAT_FP32 - 1) / ELENUM_REPEAT_FP32 * ELENUM_REPEAT_FP32;  // 128
...
__aicore__ inline void CmatinvBatchedAIV<T>::SeparateComplexMat()
{
    ...
    uint32_t alignedFpMatSize =
        (matSize * COMPLEX_ELENUM + ELENUM_BLOCK_FP32 - 1) / ELENUM_BLOCK_FP32 * ELENUM_BLOCK_FP32;
    uint32_t dstGap = 0;
    uint32_t paddingBlockNum =
        (matSize * COMPLEX_ELENUM + ELENUM_BLOCK_FP32 - 1) / ELENUM_BLOCK_FP32 * ELENUM_BLOCK_FP32;
    ...
    uint16_t repeatTime = (matSize * batchSize * matAlignedSize + ELENUM_REPEAT_FP32 - 1) / ELENUM_REPEAT_FP32;
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::CollectScalarSingle(LocalTensor<float> collectDst, uint32_t sOffset)
{
    /*
    ...
    */
    ...
    uint8_t repeatTime = (batchSize + ELENUM_REPEAT_FP32 - 1) / ELENUM_REPEAT_FP32 * COMPLEX_ELENUM;
...
__aicore__ inline void CmatinvBatchedAIV<T>::ComplexScalarDiv()
{
    ...
    */
    ...
    // compute c^2 + d^2
    Mul(this->scalarTempLocal, this->preRealLocal, this->preRealLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});
    Mul(this->resRealLocal, this->preImagLocal, this->preImagLocal, ELENUM_REPEAT_FP32, repeatTime, {1, 1, 1, 8, 8, 8});
    PipeBarrier<PIPE_V>();
    Add(this->scalarTempLocal, this->scalarTempLocal, this->resRealLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});  // c^2 + d^2
    PipeBarrier<PIPE_V>();

    // compute ac, ad, bd
    Mul(this->resRealLocal, this->afterRealLocal, this->preRealLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});  // ac
    PipeBarrier<PIPE_V>();
    Mul(this->afterRealLocal, this->afterRealLocal, this->preImagLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});  // ad
    PipeBarrier<PIPE_V>();
    Mul(this->resImagLocal, this->afterImagLocal, this->preImagLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});  // bd
    PipeBarrier<PIPE_V>();

    // compute ac + bd
    Add(this->resRealLocal, this->resRealLocal, this->resImagLocal, ELENUM_REPEAT_FP32, repeatTime, {1, 1, 1, 8, 8, 8});
    PipeBarrier<PIPE_V>();

    // compute (ac + bd) / (c^2 + d^2)
    Div(this->resRealLocal, this->resRealLocal, this->scalarTempLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});
    PipeBarrier<PIPE_V>();

    // compute bc
    Mul(this->resImagLocal, this->afterImagLocal, this->preRealLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});  // bc
    PipeBarrier<PIPE_V>();
    // compute bc - ad
    Sub(this->resImagLocal, this->resImagLocal, this->afterRealLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});
    PipeBarrier<PIPE_V>();
    // compute (bc - ad) / (c^2 + d^2)
    Div(this->resImagLocal, this->resImagLocal, this->scalarTempLocal, ELENUM_REPEAT_FP32, repeatTime,
        {1, 1, 1, 8, 8, 8});
}

template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::RealScalarVecBatchMul(LocalTensor<float> dst, LocalTensor<float> src0,
                                                                   LocalTensor<float> src1, uint32_t oneMatOffset)
{
    ...
    else
    {
        ...
        {
            Mul(dst[ELENUM_BLOCK_FP32 * i], src0[ELENUM_BLOCK_FP32 * i], src1, ELENUM_REPEAT_FP32,
                (uint8_t)(batchSize / ELENUM_BLOCK_FP32),
                {oneRowBlockStride, oneMatBlockStride, 1, static_cast<uint8_t>(ELENUM_BLOCK_FP32 * oneRowBlockStride),
...
// dst = dst - src
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::RealScalarVecBatchSub(LocalTensor<float> dst, LocalTensor<float> src,
                                                                   uint32_t oneMatOffset)
{
    uint32_t matAlignedSize = this->alignedComplexMatSize;
    uint32_t batchSize = this->batchNumPerRepeat;
    ...
    else
    {
        for (uint32_t i = 0; i < singleInstrRepeatTime; i++)
        {
            Sub(dst[ELENUM_BLOCK_FP32 * i], dst[ELENUM_BLOCK_FP32 * i], src[ELENUM_BLOCK_FP32 * i], ELENUM_REPEAT_FP32,
                (uint8_t)(batchSize / ELENUM_BLOCK_FP32),
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::ComputeAndUpdateAj(LocalTensor<float> AjReal, LocalTensor<float> AjImag,
                                                                uint32_t oneMatOffset)
{
    /*
...
template <typename T>
__aicore__ inline void CmatinvBatchedAIV<T>::RealVecScalarBatchDiv(LocalTensor<float> dst, LocalTensor<float> src0,
                                                                   LocalTensor<float> src1, uint32_t oneMatOffset)
{
    uint32_t matAlignedSize = this->alignedComplexMatSize;
    uint32_t batchSize = this->batchNumPerRepeat;  // 64 aligned, 32
    ...
    else
    {
        for (uint32_t i = 0; i < singleInstrRepeatTime; i++)
        {
            Div(dst[ELENUM_BLOCK_FP32 * i], src0[ELENUM_BLOCK_FP32 * i], src1, ELENUM_REPEAT_FP32,
                (uint8_t)(batchSize / ELENUM_BLOCK_FP32),
                                                                      ...
                                                                      LocalTensor<float> vecImag, uint32_t oneMatOffset)
{
    ...
    */
    ...
    // scalar.R^2
    Mul(scalarTempRealInner, scalarRealInner, scalarRealInner, ELENUM_REPEAT_FP32, squareSumRepeat, {1, 1, 1, 8, 8, 8});
    // scalar.I^2
    Mul(scalarTempImagInner, scalarImagInner, scalarImagInner, ELENUM_REPEAT_FP32, squareSumRepeat, {1, 1, 1, 8, 8, 8});
    PipeBarrier<PIPE_V>();
    // scalar.R^2 + scalar.I^2
    Add(scalarTempRealInner, scalarTempRealInner, scalarTempImagInner, ELENUM_REPEAT_FP32, squareSumRepeat,
        {1, 1, 1, 8, 8, 8});
    // broadcast scalar.R
    Brcb(scalarRealBrcbInner, scalarRealInner, brcbRepeatTime, {1, 8});
    // broadcast scalar.I
...
__aicore__ inline void CmatinvBatchedAIV<T>::ComplexCombinationAndCopyout(uint32_t outOffset, uint32_t batchSize)
{
    ...
    uint32_t repeatTime =
        (matRowSize * matColFpAlignedSize * batchSize * COMPLEX_ELENUM + ELENUM_REPEAT_FP32 - 1) / ELENUM_REPEAT_FP32;
...
__global__ __aicore__ void cmatinv_batched_kernel(GM_ADDR dA, GM_ADDR dUniReal, GM_ADDR dUniImag, GM_ADDR dOffset,
                                                  GM_ADDR dAinv, GM_ADDR workSpace, GM_ADDR tilingGm)
{
...
void cmatinv_batched_kernel_do(GM_ADDR dA, GM_ADDR dUniReal, GM_ADDR dUniImag, GM_ADDR dOffset, GM_ADDR dAinv,
