...
class PadCustom
{
   ...
   private:
    __aicore__ inline void CopyLines(int srcOffset, int dstOffset, int lines)
    {
        ...
        DataCopyExtParams copyInParams{static_cast<uint16_t>(lines), static_cast<uint16_t>(N * sizeof(T)),
                                       static_cast<uint16_t>((srcN - N) * sizeof(T)), 0, 0};
        ...
        DataCopyExtParams copyOutParams{static_cast<uint16_t>(lines), static_cast<uint16_t>(N * sizeof(T)), 0,
    ...
    __aicore__ inline void Process(int offsetM, int offsetN, int blockM, int realN)
    {
        if (blockM > 512)
        {
            ...
            if (!offsetM)
            {
                ...
                optranspose.Init(pipe, inQueueSrc, outQueueDst, aGlobal, workGlobal, gather1, gather2, workM, N, tileM,
                                 blockN);
...
__aicore__ inline void custom_lu(int orgM, int orgN, int blockN, int tileM, GM_ADDR A_org, GM_ADDR A_work, GM_ADDR W,
