...
__aicore__ inline void custom_lu(TBufPool<TPosition::VECCALC, 16> &tbufPool, CMatmulCustom<float> &opmm, int orgM,
                                 int orgN, int blockM, int blockN, int tileM, GM_ADDR A_work, GM_ADDR W,
                                 GM_ADDR work_gm, GM_ADDR gather1_gm, GM_ADDR gather2_gm, GM_ADDR eye_gm)
{
    int M = (orgM + 15) / 16 * 16;
    ...
    for (int i = 1, offset = blockN; i < cnt; ++i, offset += blockN)
    {
        ...
        if (blockIdx >= 4 && trsmOffsetN < trsmTotalN)
        {
            ...
            opmm.SetMatrix(aGlobalReal, aGlobalImag, aGlobalReal, aGlobalImagNeg, aGlobalImag, aGlobalReal, aGlobalImag,
                           strideN, strideN);
            opmm.Process(offset - trsmM + offset * strideN, trsmOffsetN + offset + (offset - trsmM) * strideN,
                         trsmOffsetN + offset + offset * strideN, M - offset, realBlockN, trsmM);
        }
