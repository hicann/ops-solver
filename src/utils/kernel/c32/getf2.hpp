...
class LUCustom3 {
    ...
    int lineCount;
    static constexpr int TILE_LENGTH = 8192;
    TQue<QuePosition::VECIN, 1> inQueueSrc[4];
    TQue<QuePosition::VECIN, 1> inQueueAug;
    TQue<QuePosition::VECOUT, 1> workQueue;
    LocalTensor<T> colLocal[4];
    LocalTensor<T> augLocal;
public:
    __aicore__ inline LUCustom3() {}
    __aicore__ inline void Init(TBufPool<TPosition::VECCALC, 16> *pipe, TQue<QuePosition::VECIN, 1> inQueueSrc1, TQue<QuePosition::VECIN, 1> inQueueSrc2, TQue<QuePosition::VECOUT, 1> outQueueDst, GlobalTensor<T> &aGlobal, GlobalTensor<uint32_t> &wGlobal, int M, int N, int blockN) {
        this->M = M;
        this->N = N;
        this->blockN = blockN;
        ...
        elementsPerBlock = 32 / sizeof(T);
        this->inQueueSrc[0] = inQueueSrc1;
        for (int i = 1; i < lineCount; ++i) {
            pipe->InitBuffer(this->inQueueSrc[i], 1, TILE_LENGTH * sizeof(T));
        }
        this->inQueueAug = inQueueSrc2;
        this->workQueue = outQueueDst;
    }
    __aicore__ inline void Process(int MatAOffset, int offsetM, int offsetN, int blockM, int realM, int realN) {
        this->blockM = blockM;
        this->realM = realM;
        ...
        for (int k = 0; k < t; ++k) {
            ...
            if (k >= startLine && k < endLine) {
                ...
                Barrier();
                while (wGlobal.GetValue(offsetM + k) == -1) {
                    DataCacheCleanAndInvalid<uint32_t, CacheLine::SINGLE_CACHE_LINE, DcciDst::CACHELINE_OUT>(wGlobal[offsetM + k]);
                    Barrier();
                }
...
public:
    ...
    __aicore__ inline void Init(TBufPool<TPosition::VECCALC, 16> *pipe, TQue<QuePosition::VECIN, 1> inQueueSrc1, TQue<QuePosition::VECIN, 1> inQueueSrc2, TQue<QuePosition::VECOUT, 1> outQueueDst, GlobalTensor<T> aGlobal, GlobalTensor<T> workGlobal, GlobalTensor<uint32_t> gather1, GlobalTensor<uint32_t> gather2, int M, int N, int tileM, int tileN) {
        this->aGlobal = aGlobal;
...
class SwapRows {
    static constexpr int TILE_LENGTH = 8192;
    GlobalTensor<T> aGlobal, eyeGlobal;
...
public:
    ...
    __aicore__ inline void Init(TBufPool<TPosition::VECCALC, 16> *pipe, GlobalTensor<T> aGlobal, GlobalTensor<T> eyeGlobal) {
