    ...
    __aicore__ inline void Process(int offsetM, int offsetN, int blockM, int realN) {
        if (blockM > 512) {
            ...
            if (!offsetM) {
                ...
                optranspose.Init(pipe, inQueueSrc1, inQueueSrc2, outQueueDst, aGlobal, workGlobal, gather1, gather2, workM, N, tileM, blockN);
...
__aicore__ inline void custom_lu(TBufPool<TPosition::VECCALC, 16> &tbufPool, MatmulCustom<float> &opmm, int orgM, int orgN, int blockM, int blockN, int tileM, GM_ADDR A_work, GM_ADDR W, GM_ADDR work_gm, GM_ADDR gather1_gm, GM_ADDR gather2_gm, GM_ADDR eye_gm) {
    int M = (orgM + 15) / 16 * 16;
